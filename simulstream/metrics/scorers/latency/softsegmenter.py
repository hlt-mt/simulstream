# Copyright 2025 Peter Polak (polakpe7@gmail.com)
...
class ResegmentedLatencyScoringSample:
    """
    A sample containing realigned hypotheses and references using SoftSegmenter.

    Attributes:
        audio_name (str): The identifier of the audio file.
        hypothesis (List[OutputWithDelays]): Hypothesis lines after realignment.
        reference (List[ReferenceSentenceDefinition]): Reference lines aligned to the hypothesis.
    """
    audio_name: str
...
def metric(ref_word: Word, hyp_word: Word, char_level: bool) -> float:
    """
...
def tokenize_words(words: List[Word], lang: Optional[str]) -> List[Word]:
    """
...
class SoftSegmenterBasedLatencyScorer(LatencyScorer):
    ...
    def _create_words_from_output(
        ...
        assert len(units) == len(output.computational_aware_delays), \
            f"Number of units ({len(units)}) and CA delays ({len(output.computational_aware_delays)}) do not match"

        words = []
        for unit, delay, elapsed in zip(units, output.ideal_delays, output.computational_aware_delays):
            words.append(Word(
                text=unit,
                delay=delay,
                elapsed=elapsed,
    ...
    def score(self, samples: List[LatencyScoringSample]) -> LatencyScores:
        ...
        for sample in samples:
            ...
            ref_words = tokenize_words(ref_words, self.moses_lang)

            # Convert hypothesis to Word objects
            hyp_words = self._create_words_from_output(
                sample.hypothesis, char_level, recording_length)
            hyp_words = tokenize_words(hyp_words, self.moses_lang)

            # Align sequences
            aligned_ref, aligned_hyp = align_sequences(ref_words, hyp_words, char_level)

            # Process alignment to assign sequence IDs
            processed_hyp = process_alignment(aligned_ref, aligned_hyp, char_level)

            # Group hypothesis words by sequence ID
            new_segmentation = {}
            for ref in sample.reference:
                idx = sample.reference.index(ref)
                new_segmentation[idx] = []
