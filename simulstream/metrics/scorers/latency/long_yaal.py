...
class LongYAAL(SoftSegmenterBasedLatencyScorer):
    ...
    def _sentence_level_yaal(
            ...
            is_longform: bool = True) -> Optional[float]:
        """
        ...
        Args:
            ...
            is_longform (bool): Whether to treat as long-form audio (allows delays past source_length).

        Returns:
            Optional[float]: The YAAL score for the sentence, or None if computation is not possible.
        """
        if len(delays) == 0:
            return None
    ...
    def _do_score(self, samples: List[ResegmentedLatencyScoringSample]) -> LatencyScores:
        ...
        for sample in samples:
            ...
            ) if sample.reference else 0.0
            
            for sentence_output, sentence_reference in zip(sample.hypothesis, sample.reference):
                # Note: delays in sentence_output are already offset relative to sentence_reference.start_time
                ...
                if len(ideal_delays) > 0:
                    ...
                    # This allows LongYAAL to account for outputs emitted after the reference segment ends
                    # but before the recording ends (key difference from StreamLAAL)
        ...
        return LatencyScores(
            ...
            statistics.mean(sentence_level_ca_scores) if len(sentence_level_ca_scores) > 0 else float('nan')
        )
